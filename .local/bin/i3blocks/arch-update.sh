...
aur_update_count = 0
if args.aur:
    aur_update_count = len(get_aur_updates())

if update_count > 0 or aur_update_count > 0:
    info = str(update_count)
    if aur_update_count > 0:
            info += "," + str(aur_update_count)
    short_info = info
    info += ' updates'
    ...
    print(message.format(label, args.base_color, 'no updates'))
